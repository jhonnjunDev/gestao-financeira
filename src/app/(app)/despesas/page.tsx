...
export default async function DespesasPage() {
  ...
  const pending = expenses.filter((t) => t.status === "PENDING").reduce((s, t) => s + Number(t.amount), 0);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">{expenses.length} registros · Total: {formatCurrency(total)} · Pendente: {formatCurrency(pending)}</p>
        </div>
              ...
              {expenses.map((r) => {
                ...
                return (
                  <tr key={r.id} className="hover:bg-gray-50">
                    ...
                    <td className="table-cell"><Badge variant={s.label === "Pago" ? "success" : s.label === "Atrasado" ? "danger" : "warning"}>{s.label}</Badge></td>
                    <td className="table-cell text-right font-semibold text-red-600">{formatCurrency(r.amount)}</td>
                  </tr>
                );
              })}
              {expenses.length === 0 && (
                <tr><td colSpan={6} className="text-center py-12 text-gray-400">Nenhuma despesa registrada</td></tr>
              )}
            </tbody>
          </table>
        </div>
