...
export default async function ContasAReceberPage() {
  ...
  const pending = await prisma.transaction.findMany({
    where: { accountId: account.id, type: "INCOME", status: { in: ["PENDING", "LATE"] }, deletedAt: null },
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">{pending.length} receitas pendentes · Total: {formatCurrency(total)}</p>
        </div>
        <a href="/movimentacoes" className="btn-primary">Nova receita</a>
      </div>
      ...
      <Card padding={false}>
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {pending.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-12 text-gray-400">Nenhuma receita pendente</td></tr>
              ) : (
                pending.map((t) => {
                    ...
                    <tr key={t.id} className="hover:bg-gray-50">
                      ...
                      <td className="table-cell">
                        <span className="text-gray-500">
                          {dd !== null ? (dd < 0 ? `${Math.abs(dd)} dias atrasado` : dd === 0 ? "Hoje" : `${dd} dias`) : "-"}
                        </span>
                      </td>
                      <td className="table-cell"><Badge variant="warning">Pendente</Badge></td>
                      <td className="table-cell text-right font-semibold text-emerald-600">{formatCurrency(t.amount)}</td>
                    </tr>
                  );
                })
              )}
